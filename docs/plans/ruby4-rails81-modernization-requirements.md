...
|----|-------------|----------|-------|
| R1 | The gem MUST declare Ruby `>= 4.0` and MUST execute on both the Ruby 4.0.0 support floor and target Ruby 4.0.5 runtime without compatibility warnings caused by Litestack's own code or test doubles. | Must Have | Ruby versions below 4.0 are unsupported; directly required bundled gems `logger`, `base64`, and `bigdecimal` are explicit runtime dependencies. |
...
| R10 | Connection, pool, statement, worker, fork, scheduler, and `at_exit` lifecycle operations MUST be idempotent on Ruby 4 and sqlite3 2.x; repeated close/shutdown calls MUST NOT raise, leak any pooled resource, reopen a closed object after fork, or leave background execution contexts running. | Must Have | Scheduler selection is current-thread based; workers are explicitly wakeable; bounded shutdown never closes SQLite under a live task; timeout leaves a named retryable state with structured diagnostics. |
| R11 | Existing durable data created by the current 0.4.x format for Litedb, Litejob, Litesearch, LiteKD, and Litemetric MUST open unchanged or upgrade without data loss. | Must Have | Upgrade requires old processes to be quiesced, acquires bounded advisory and SQLite write locks, uses a separate read source and destination for online backup, performs validated transactional schema changes, protects Litesearch rebuild/`WRITABLE_SCHEMA` paths, publishes a WAL-consistent integrity/foreign-key-checked snapshot without overwrite before a destructive transformation, preserves restrictive permissions, emits structured progress/failure events, and leaves the old file usable after failure; downgrade compatibility is not required. |
...
- Ruby 4 scheduler/no-scheduler thread and fork matrices remain warning-free; closing a multi-connection pool drains every resource, an unwakeable worker never loses its live database underneath it, and implicit/explicit-exit subprocesses finish within the configured bound without lifecycle stderr.
...
- Versioned 0.4.x fixture databases for every durable component named in R11 upgrade successfully with record/count/content assertions; three-handle online backup captures committed WAL state, full integrity/foreign-key/semantic checks pass before no-replace publication, and injected backup/migration failures demonstrate rollback and preservation of the original file.
- CI contains blocking lower-bound, exact-target, latest-supported-release, minimum-Ruby/latest-Rails, and latest-Ruby/minimum-Rails jobs; every combination runs dependency resolution and the full suite, exact/latest jobs cover style, build, install, and smoke verification, and optional head/main jobs are visibly non-blocking.
